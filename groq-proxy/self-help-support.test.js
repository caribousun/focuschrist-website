...
try {
 ...
 const result=await(await worker.fetch(request(payload(question)),{OPENAI_API_KEY:'fixture-only'})).json();assert(provider>0&&researchBodies.length>0,'Legitimate support reaches stub research/provider path');assert(researchBodies.every(body=>body.instructions.includes('do not compose an answer')&&JSON.stringify(body.input).includes(question)),'Discovery stays metadata-only and researches actual support question');assert.notEqual(result.focuschrist_gateway_mode,'respect-boundary');assert.equal(result.focuschrist_source_integrity_verified,false,'No-evidence stub cannot publish verified advice');assert.equal(result.choices[0].message.content,SOURCE_UNAVAILABLE_MESSAGE,'Existing evidence failure stays closed');
