...
const SOURCE_POLICY_VERSION = '2026-09-13.88';
const OFFICIAL_EXCERPT_CACHE_VERSION = '2026-09-13.88';
...
function reviewedDeterministicEvidenceRecovery(question, evidence, page = '') {
  ...
  if (['ask', 'church-history'].includes(page)
    && /^\s*What occurred around the 1836 dedication of the Kirtland Temple\?\s*$/i.test(currentQuestion)) {
    const sourceIndex = sources.findIndex((source) => {
      let parsed;
      try { parsed = new URL(String(source && source.url || '')); } catch (_error) { return false; }
      const content = String(source && source.content || '');
  ...
  if (isPioneerIrrigationIntent(value, page)) {
    const sourceIndex = sources.findIndex((source) => {
      ...
      if (parsed.protocol !== 'https:'
        ...
        || parsed.pathname !== '/study/manual/church-history-in-the-fulness-of-times/chapter-twenty-six') return false;
      const content = String(source && source.content || '');
      return /\birrigat\w*\b/i.test(content)
        && /\bsalt\s+lake\s+valley\b/i.test(content)
        && /\bplant\w*\b/i.test(content)
        && /\b(?:settle\w*|communit\w*|pioneer\w*)\b/i.test(content);
    });
    if (sourceIndex >= 0) return {
      recoveryId: 'reviewed-pioneer-irrigation-settlement',
      answer: REVIEWED_PIONEER_IRRIGATION,
  ...
  if (/\bhyrum\s+smith\b/i.test(value)
    && (/\b(?:leader\w*|responsibilit\w*|office|offices|role|roles|service|serve\w*)\b/i.test(currentQuestion)
      || /\bwho\s+(?:is|was)\s+hyrum\s+smith\b/i.test(currentQuestion))) {
    const sourceIndex = sources.findIndex((source) => {
      let parsed;
      try { parsed = new URL(String(source && source.url || '')); } catch (_error) { return false; }
      if (parsed.protocol !== 'https:'
        || !(parsed.hostname === 'churchofjesuschrist.org' || parsed.hostname.endsWith('.churchofjesuschrist.org'))
        || parsed.pathname !== '/study/history/topics/hyrum-smith') return false;
      const content = String(source && source.content || '');
      return /\bhyrum\s+smith\b/i.test(content)
        && /\bchurch\s+patriarch\b/i.test(content)
        && /\bassistant\s+president\s+of\s+the\s+church\b/i.test(content);
    });
    if (sourceIndex >= 0) {
      return {
        recoveryId: 'reviewed-hyrum-smith-leadership',
        answer: REVIEWED_HYRUM_SMITH_LEADERSHIP,
        sourceIndexes: [sourceIndex + 1],
      };
    }
  }
