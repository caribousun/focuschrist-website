...
- Live `.13` acceptance initially encountered the research model's daily token limit. The pre-existing low-risk general fallback also rejected its own correct but short Lincoln draft under the new depth gate. The low-risk path now performs one depth expansion with the same stable-knowledge checker, so ordinary durable facts remain complete during research-model rate limits while current, high-stakes, specialized, quotation, citation, and source-specific questions remain fail-closed.
