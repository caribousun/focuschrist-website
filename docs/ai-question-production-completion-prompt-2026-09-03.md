...
- Candidate `.34` keeps the two-term receipt gate unchanged. It makes cache packing and later excerpt selection aware of the already bounded topic pin so direct irrigation and Pioneer settlement paragraphs outrank generic chapter paragraphs, includes the current query when compacting a fresh page, and changes the official-excerpt cache namespace so stale packs cannot survive deployment. Cache size, source count, fetch count, official-domain, HTML, deadline, provider, safety, answer, and capacity limits remain unchanged. Unit coverage requires the packed and selected chapter evidence to retain both concepts.
