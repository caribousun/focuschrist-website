...
- Exact `.33` deployed at `baf96977d56eb3d40894cd93f0d17fe06264542f`; all 15 core answers completed, including the repaired short Pioneer request. Acceptance still failed because the cached chapter 26 excerpt carried only one normalized relevance term. Candidate `.34` preserves the two-term gate, prioritizes direct pinned-topic paragraphs during cache packing and excerpt selection, and versions the excerpt cache namespace to remove stale packs. Production remains `VERIFIED FAIL`.
