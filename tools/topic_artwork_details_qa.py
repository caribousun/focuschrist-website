...
def parents(n):
 while n.parent:
  n=n.parent;yield n
errors=[];count=0;preserved=0;panels=0;life_assets=[]
for page in [*sorted((ROOT/'answers').glob('*.html')),ROOT/'general-conference.html']:
 ...
 for a in [n for n in ns if n.tag=='a' and 'href' in n.attrs and (n.parent.tag=='figure' or n.has('fc-marriage-era__art') or n.has('fc-foundation-card__image'))]:
  ...
  if 'data-life-after-death-art' in container.attrs:
   ...
   life_assets.append((page.parent/urlsplit(a.attrs['href']).path).resolve().relative_to(ROOT).as_posix())
...
assert (count-len(life_assets),preserved)==(99,3),(count,preserved)
